(function () {
  ...
  const API_BASE_PATH = `${window.API_URL}/api`;
  ...
  const apiClient = {
    ...
    async fetchProduct(id) {
      return request(`/public/productos/${id}`);
    },
    async buscarProductos(termino) {
      return request(`/public/productos/buscar?q=${encodeURIComponent(termino)}`);
    },
    /**
     * Creates an order and returns the payment preference.
     * @param {object} data - The order data.
     * @returns {Promise<object>} A promise that resolves to the preference response.
     */
    async crearOrden(data) {
      return withAuthenticatedRequest('/user/payments/crearOrden', 'POST', data);
    }
  };
