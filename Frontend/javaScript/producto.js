(function() {
  ...
  function normalizeType(value, isDecant) {
    ...
    if (safe.includes("vape")) return "vape";
  ...
  async function fetchProductById(type, id) {
    ...
    try {
      if (type === "vape") {
        if (client?.fetchVape) {
          return await client.fetchVape(id);
        }
        const response = await fetchJson(`${PUBLIC_PATH}/vapes/${id}`);
        return response;
      }
  ...
  function buildFeatures({ type, ml, pitadas, base, stored, genero, sabores }) {
    const volume = base?.volumen || stored?.volumen || (ml ? `${ml} ml` : "No especificado");
    const duration = base?.duracion || stored?.duracion || (type === "vape" && pitadas ? `${pitadas} puffs` : "No especificado");
    const projection = base?.proyeccion || stored?.proyeccion || (type === "vape" ? "Alta densidad de vapor" : "No especificado");
    const season = base?.temporada || stored?.temporada || "No especificado";
    const occasion = base?.ocasion || stored?.ocasion || (genero ? `Ideal para público ${capitalize(genero.toLowerCase())}` : "No especificado");

    if (type === "vape" && sabores.length) {
      return {
        concentration: sabores.slice(0, 3).join(", ") + (sabores.length > 3 ? "…" : ""),
        duration,
        projection,
        season: "Uso diario",
        occasion: occasion === "No especificado" ? "Momentos de relax" : occasion
      };
    }
  ...
  function buildHighlights({ type, genero, ml, pitadas, notasPrincipales, transferPrice, price, sabores }) {
    const highlights = [];
    if (genero) highlights.push(`Pensado para público ${capitalize(genero.toLowerCase())}`);
    if (ml) highlights.push(`${ml} ml disponibles`);
    if (type === "vape" && pitadas) highlights.push(`Hasta ${pitadas} puffs`);
    if (type === "vape" && sabores.length) highlights.push(`Sabores destacados: ${sabores.slice(0, 2).join(" y ")}`);
    ...
    if (!highlights.length) {
      highlights.push("Garantía de originalidad Vesper");
      highlights.push("Envíos a todo el país");
    }
    return highlights;
  }
  ...
  function updateBreadcrumb(elements, detail) {
    ...
    const categoryLabel = detail.type === "vape"
      ? "Vapes"
      : detail.type === "decant"
        ? "Decants"
        : "Perfumes";
  ...
  async function fetchCatalog() {
    ...
    const load = async () => {
      try {
        ...
        const vapesPromise = client?.fetchVapes ? client.fetchVapes() : fetchJson(`${PUBLIC_PATH}/vapes`);
  ...
  function normalizeVape(vape) {
    if (!vape) return null;
    const images = extractImages(vape?.imagenes);
    const price = Number.parseFloat(vape?.precio) || 0;
    const sabores = Array.isArray(vape?.sabores)
      ? vape.sabores.map((s) => s?.nombre || s?.name || "").filter(Boolean)
      : [];
    const flavorTokens = new Set([
      ...tokenize(sabores.join(" ")),
      ...tokenize(vape?.descripcion)
    ]);
    const subtitleParts = [];
    if (Number.isFinite(vape?.pitadas)) subtitleParts.push(`${vape.pitadas} puffs`);
    if (sabores.length) subtitleParts.push(`Sabores: ${sabores.slice(0, 2).join(" y ")}`);
  ...
  function computeSimilarity(detail, candidate) {
    ...
    if (detail.type !== "vape" && candidate.tokens) {
    ...
    if (detail.type === "vape" && candidate.sabores) {
      const sharedFlavors = intersectCount(new Set(tokenize(detail.sabores?.join(" ") || "")), candidate.tokens || new Set());
      score += sharedFlavors * 2;
      if (detail.pitadas && candidate.pitadas) {
        score += Math.max(0, 1 - Math.abs(detail.pitadas - candidate.pitadas) / 5000);
      }
    }
  ...
  async function renderRelatedProducts(container, detail) {
    ...
    try {
      ...
      const poolRaw = detail.type === "vape" ? catalog.vapes : catalog.perfumes;
      const normalizer = detail.type === "vape" ? normalizeVape : normalizePerfume;
