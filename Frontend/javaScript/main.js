      ...
      const loadData = async () => {
        try {
          const perfumesPromise = window.apiClient?.fetchPerfumes?.()
            ?? fetch('/api/public/perfumes').then((res) => res.json());
  ...
  async function renderFeaturedProductsSection(container) {
    ...
    try {
      ...
      const destacados = destacadosRaw.map((d) => {
        ...
        if ((type === "perfume" || type === "decant") && perfumeMap.has(id)) {
          return normalizePerfume(perfumeMap.get(id));
        }
        // If type is unknown or mismatched, try id on perfumes map
      ...
      try {
        if (needsHydration.length) {
          ...
          if (catalog && Array.isArray(catalog.perfumes)) {
            const perfumeMap = new Map((catalog.perfumes || []).map(p => [p.id, p]));
            for (const entry of needsHydration) {
              ...
              if (!id) continue;
              const fromCatalog = type === "perfume" ? perfumeMap.get(id) : null;
              if (fromCatalog) {
      ...
      for (const entry of needsHydration.filter(e => !e.resolved)) {
        try {
          ...
          } else if (typeof window.apiClient.fetchPerfume === "function") {
