(function () {
  ...
  function renderAddressList() {
    ...
    state.domicilios.forEach((domicilio) => {
      ...
      title.textContent = `${domicilio.nombre} ${domicilio.apellido}`;
      const addressLine = document.createElement('p');
      addressLine.textContent = `${domicilio.calle} ${domicilio.numero}`;
      ...
      body.innerHTML = [`Entre calles: ${domicilio.entreCalles}`]
        .concat(domicilio.observaciones ? [`Observaciones: ${domicilio.observaciones}`] : [])
        .map((text) => `<p>${text}</p>`)
        .join('');
