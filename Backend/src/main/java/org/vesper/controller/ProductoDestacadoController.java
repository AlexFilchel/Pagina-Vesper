...
public class ProductoDestacadoController {
    ...
    public ResponseEntity<ProductoDestacadoResponse> agregarProductoDestacado(
            @Valid @RequestBody ProductoDestacadoRequest request) {
        return ResponseEntity.ok(productoDestacadoService.agregarDestacado(request));
    }

    @DeleteMapping("/admin/productos-destacados/{id}")
    public ResponseEntity<Void> eliminarProductoDestacado(@PathVariable Long id) {
